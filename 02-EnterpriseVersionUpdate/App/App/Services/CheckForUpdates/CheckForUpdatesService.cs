...
namespace App.Services.CheckForUpdates
{
    public class CheckForUpdatesService
    {
        ...
        public UpdateChangesResponse GetVersionChanges() => CreateMockUpdateChangesResponse();
        ...
        private UpdateChangesResponse CreateMockUpdateChangesResponse()
        {
            var result = new UpdateChangesResponse
            {
                ...
                UpdateLinkUri = "itms-services://?action=download-manifest&url=itms-services://?action=download-manifest&url=https://example.com/manifest.plist",
            ...
            // Iterate through the mock version changes and remove those for versions older than the currently installed one.
            // This could be handled in a web service request by passing the currently installed version number in the request.
            if (int.TryParse(ToDigitsOnly(result.CurrentVersion), out int currentVersionDigitsOnly))
            {
                result.VersionChanges = result.VersionChanges.Where((versionChanges) =>
                {
                    var include = false;
